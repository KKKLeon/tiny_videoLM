...
@registry.register_model("timechat") #TimeChat=registry.register_model("timechat")(TimeChat)
class TimeChat(Blip2Base):
    ...
    def __init__(
        ...
        #)
        self.visual_encoder, self.ln_vision = self.init_vision_encoder_umt(vit_cfg)
        #print("visual encoder img size is ",vit_cfg.img_size)
        #print("visual encoder emd is ", self.visual_encoder.encoder_embed_dim)
        if freeze_vit:
        ...
        if self.low_resource:
            self.llama_model = LlamaForCausalLM.from_pretrained(
                ...
                load_in_8bit=True,
                device_map={'': device_8bit} #device_map?
            )
        else:
            if max_txt_len > 2048:
        ...
        if prompt_path:
            ...
            filted_prompts = [raw_prompt for raw_prompt in raw_prompts if "<ImageHere>" in raw_prompt] #含图片的prompt
            self.prompt_list = [prompt_template.format(p) for p in filted_prompts]
            print('Load {} training prompts'.format(len(self.prompt_list)))
            print('Prompt Example \n{}'.format(random.choice(self.prompt_list)))
        else:
            self.prompt_list = []
        #随机生成一个长度为max_frame_pos,维度为hidden_size的映射表
    ...
    def encode_videoQformer_visual(self, image, timestamp=None):
        ...
        with self.maybe_autocast(): #半精度加速训练
            # embed image features with blip2, out: (b t) q h
            image_embeds = self.ln_vision(self.visual_encoder(image)).to(device) # (b t), tokens/image, embed_hidden
            print(f"Embeded images dimension is {image_embeds.shape}")
            ...
            if self.qformer_text_input:
                ...
                query_atts = torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(image.device)#(1,num of query)
            ...
            position_ids = torch.arange(time_length, dtype=torch.long, device=query_tokens.device) #使用了 PyTorch 的 torch.arange() 函数来生成一个从 0 到 time_length-1 的整数序列
            position_ids = position_ids.unsqueeze(0).expand(batch_size, -1)
            frame_position_embeddings = self.video_frame_position_embedding(position_ids) #(b,t,1, h)
            ...
            if self.window_size <= 0:
                ...
                frame_hidden_state = einops.rearrange(frame_hidden_state, 'b t q h -> b (t q) h', b=batch_size,
                                                      t=time_length)
                frame_atts = torch.ones(frame_hidden_state.size()[:-1], dtype=torch.long).to(device) #(b,(t q))
                video_query_tokens = self.video_query_tokens.expand(frame_hidden_state.shape[0], -1,
                                                                    -1)  # expand on batch dim (b,q,h）
    ...
    def from_config(cls, cfg):
        ...
        ckpt_path = cfg.get("ckpt", "")  # load ckpt/timechat/timechat_7b.pth
        if ckpt_path:
