...
from timechat.models.uml import build_vit
class Blip2Base(BaseModel):
    ...
    def init_Qformer(cls, num_query_token, vision_width, cross_attention_freq=2):
        ...
        encoder_config.query_length = num_query_token #num_query
        Qformer = BertLMHeadModel(config=encoder_config)
        query_tokens = nn.Parameter(
            torch.zeros(1, num_query_token, encoder_config.hidden_size)
        ) # nn.Parameter()：将上述创建的全零张量包装为一个torch.Parameter对象，使其成为模型的可学习参数
    ...
    @classmethod
    def init_vision_encoder_umt(cls, config):
        ...
        if config.vit_add_ln:
            vision_layernorm = nn.LayerNorm(config.encoder_embed_dim, eps=1e-12)
        else:
    ...
    def load_from_pretrained(self, url_or_filename):
        ...
        msg = self.load_state_dict(state_dict, strict=False) #load_state_dict方法会返回一个包含有关加载操作的消息的字符串。这个消息可能会提供一些关于加载过程的信息，例如哪些参数被成功加载，哪些参数由于形状或名称不匹配而被忽略等。
