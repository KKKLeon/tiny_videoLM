...
def load_video(video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", return_msg = False):
    decord.bridge.set_bridge("torch") # 将视频数据转换为 Torch 张量
    vr = VideoReader(uri=video_path, height=height, width=width) #读取视频数据
...
class AlproVideoEvalProcessor(AlproVideoBaseProcessor):
    def __init__(self, image_size=256, mean=None, std=None, n_frms=MAX_INT):
        ...
        self.transform = transforms.Compose(
            [
                ...
                ToUint8(),  # C, T, H, W
                ToTHWC(),  # T, H, W, C
                transforms_video.ToTensorVideo(),  # C, T, H, W 而且归一化/255
    ...
    def from_config(cls, cfg=None):
        ...
        image_size = cfg.get("image_size", 256) #224

        mean = cfg.get("mean", None)
        std = cfg.get("std", None)

        n_frms = cfg.get("n_frms", MAX_INT) #96
