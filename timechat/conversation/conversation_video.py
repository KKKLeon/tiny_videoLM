        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                ...
                if message:
                    ...
                    ret += ""
            ret = ret.lstrip(self.sep) # 从ret字符串的开头开始移除所有在self.sep中指定的字符
            return ret
        else:
...
class Chat:
    ...
    def ask(self, text, conv):
        if len(conv.messages) > 0 and conv.messages[-1][0] == conv.roles[0] \
                and (
                '</Video>' in conv.messages[-1][1] or '</Image>' in conv.messages[-1][1]):  #conv.roles[0]='user', last message is image.
